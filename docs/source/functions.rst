 ...
 * ``report_key`` - The name of whichever report you prefer the baseline currency to be taken from.
...
So, the first step is to register the original data into a ``DateValueData()``. In our previous examples the original data is: Net Income, Revenue and Total Equity. Let's see how we do that.
